...
void classify_KNN(float *X_train, float *y_train, float *X_test, float *y_test, int rank, int size,int TRAINING_SIZE, int TESTING_SIZE, int NO_OF_FEATURES,int NO_OF_CLASSES, int n_neighbors){
	int i, j,predictions,data_size_process, row_size_process, a;
	float *data_per_process, *overall_distance, *distance_per_process,*labels_per_process,*overall_labels, *t_labels_per_process;
	...
	row_size_process = TRAINING_SIZE/size; //135/3=45
	data_size_process = row_size_process*NO_OF_FEATURES; //45*4=180

	data_per_process = dynamicArray(data_size_process, 1);//180
	distance_per_process = dynamicArray(row_size_process, 1);//45
	overall_distance = dynamicArray(TRAINING_SIZE, 1);//135

	labels_per_process = dynamicArray(row_size_process, 1);//45
	t_labels_per_process = dynamicArray(row_size_process, 1);//45
	overall_labels = dynamicArray(TRAINING_SIZE, 1);//135

	MPI_Scatter(X_train, data_size_process, MPI_FLOAT, data_per_process, data_size_process, MPI_FLOAT, 0,  MPI_COMM_WORLD);
	printf("\t\t\tINPUT DATA(X)\n");
	printf("INPUT DATA(X) FOR PROCESSOR %d\n", rank);
	for(i=0;i<data_size_process;i=i+1)
	{
		printf("%.2f ", data_per_process[i]);
		if(i%NO_OF_FEATURES==0)
			printf("\n");
		
	}
	printf("\n");
	MPI_Scatter(y_train, row_size_process, MPI_FLOAT, t_labels_per_process, row_size_process, MPI_FLOAT, 0,  MPI_COMM_WORLD);
	printf("\t\t\tINPUT DATA(Y)\n");
	printf("INPUT DATA(Y) FOR PROCESSOR %d\n", rank);
	for(a=0;a<row_size_process;a=a+1)
	{
		printf("%.2f ", t_labels_per_process[a]);
	}
	printf("\n");

	float *x = dynamicArray(NO_OF_FEATURES, 1);//4
	for (i=0; i<TESTING_SIZE; i=i+1){//15	
		...
		printf("LOCAL RESULTS OF PROCESSOR %d\n", rank);
		for(i=0;i<row_size_process;i++)
		{
		printf("Distance= %.2f , Label= %.2f\n ", distance_per_process[i], labels_per_process[i]);
		}
